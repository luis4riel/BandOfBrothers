...
namespace BancoTabajara.Controller.Tests.Funcionalidades.Contas
{
	...
	public class ContasControllerTests:TestControllerBase
	{
        ...
        private Mock<ContaQuery> _contaQuery;
        private Mock<ContaRegistroCommand> _contaInserir;
        private Mock<ContaAtualizarCommand> _contaAtualizar;
        private Mock<ContaRemoveCommand> _contaRemove;
		...
		public void Initialize()
		{
            ...
            _contaQuery = new Mock<ContaQuery>();
            _contaInserir = new Mock<ContaRegistroCommand>();
            _contaAtualizar = new Mock<ContaAtualizarCommand>();
            _contaRemove = new Mock<ContaRemoveCommand>();
        }
		...
		public void Conta_Controller_PegarTodos_Deve_Funcionar()
		{
            ...
            _contaQuery.Object.Estado = conta.Estado;
            _contaQuery.Object.Id = conta.Id;
            _contaQuery.Object.Limite = conta.Limite;
            _contaQuery.Object.Titular = conta.Titular;

            var response = new List<ContaQuery>() { _contaQuery.Object }.AsQueryable();
		...
		public void Conta_Controller_PegarPorId_DeveFuncionar()
		{
			...
			_contaServicoMock.Setup( c => c.PegarPorId( id ) ).Returns( _contaQuery.Object );
		...
		{
			int id = 1;
			_contaServicoMock.Setup( c => c.Inserir( _contaInserir.Object ) ).Returns( id );

			IHttpActionResult callback = _contasController.Inserir( _contaInserir.Object );

			var httpResponse = callback.Should().BeOfType<OkNegotiatedContentResult<int>>().Subject;
			httpResponse.Content.Should().Be( id );
			_contaServicoMock.Verify( c => c.Inserir( _contaInserir.Object ), Times.Once );
		}
		...
		{
			var atualizado = true;
			_contaServicoMock.Setup( c => c.Atualizar( _contaAtualizar.Object ) ).Returns( atualizado );

			IHttpActionResult callback = _contasController.Atualizar( _contaAtualizar.Object );

			var httpResponse = callback.Should().BeOfType<OkNegotiatedContentResult<bool>>().Subject;
			httpResponse.Content.Should().BeTrue();
			_contaServicoMock.Verify( s => s.Atualizar( _contaAtualizar.Object ), Times.Once );
		}
		...
		public void Conta_Controller_Put_Id_nao_encontrado_deve_retornar_excecao()
		{
			...
			_contaServicoMock.Setup( c => c.Atualizar( _contaAtualizar.Object ) ).Throws<NaoEncontradoException>();
		...
		{
			var atualizado = true;
			_contaServicoMock.Setup( c => c.Deletar( _contaRemove.Object ) ).Returns( atualizado );

			IHttpActionResult callback = _contasController.Deletar( _contaRemove.Object );

			var httpResponse = callback.Should().BeOfType<OkNegotiatedContentResult<bool>>().Subject;
			_contaServicoMock.Verify( s => s.Deletar( _contaRemove.Object ), Times.Once );
			httpResponse.Content.Should().BeTrue();
		}
